...
for sim in `eval echo {1..${no_sims}}`; do
	...
	cd /home/svendvn/git/IMCoalHMM/simulations/ILS
	fsc25 -i parameters2.par -n 1
	cd /home/svendvn
	mv /home/svendvn/git/IMCoalHMM/simulations/ILS/parameters2 ${simdir}
	python /home/svendvn/git/IMCoalHMM/scripts/arlequin_to_fasta2.py ${simdir}/parameters2/parameters2_1_1.arp 1000000 ${simdir}
	cat ${simdir}/sample_2_1.fasta >> ${simdir}/sample_1_1.fasta
	cat ${simdir}/sample_3_1.fasta >> ${simdir}/sample_1_1.fasta
	seqfile=${simdir}/sample_1_1.fasta

	echo "data made"
        python /home/svendvn/git/IMCoalHMM/scripts/prepare-alignments.py --names=1,2,3 ${seqfile} fasta ${ziphmmfile} --where_path_ends 3
	...
	python /home/svendvn/git/IMCoalHMM/scripts/ILS3.py -o INMmcmc-smallVar-sim-${sim}-chain.txt --alignments ${simdir}/*.ziphmm --parallels 12 --fix_params 7 --fix_parameters_to_be_equal=2,3,4,5
