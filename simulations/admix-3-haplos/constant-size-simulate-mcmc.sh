...
for sim in `eval echo {1..${no_sims}}`; do
    ...
    for chunk in `eval echo {1..${no_chunks}}`; do
        ...
        ziphmmfile11=${simdir}/alignment.$chunk.ziphmm
	
	cd /home/svendvn/workspace/IMCoalHMM/simulations/admix-3-haplos
	fsc25 -i parameters.par -n 1
	cd /home/svendvn
	mv /home/svendvn/workspace/IMCoalHMM/simulations/admix-3-haplos/parameters ${simdir}
	python /home/svendvn/workspace/IMCoalHMM/simulations/admix-3-haplos/arlequin_to_fasta2.py ${simdir}/parameters/parameters_1_1.arp ${seg_length} ${simdir}
	...
	echo "data made"
        python /home/svendvn/workspace/IMCoalHMM/scripts/prepare-alignments.py --names=1,2,3 ${seqfile} fasta ${ziphmmfile11} --where_path_ends 3
	...
	python /home/svendvn/workspace/IMCoalHMM/scripts/admix_3_haplos.py -o INMmcmc-smallVar-sim-${sim}-chain.txt --alignments ${simdir}/*.ziphmm \
		--samples 100 --thinning 1  --sd_multiplyer 0.1 --adap 1 --mc3 --parallels 2 --adap_desired_accept 0.234 --no_mcmc
