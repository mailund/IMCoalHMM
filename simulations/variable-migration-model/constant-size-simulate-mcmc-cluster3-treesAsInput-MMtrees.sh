...
for sim in `eval echo {1..${no_sims}}`; do
    ...
    for chunk in `eval echo {1..${no_chunks}}`; do
        ...
        python /scratch/$SLURM_JOBID/simulate_variable_migration_trees.py -o ${simdir}/ --type 0
	...
	python /scratch/$SLURM_JOBID/variable-migration-model-mcmc.py -o INMmcmc-$var-simTrees-${sim}-chain.txt --treefile $treefile --samples 10000 --thinning 10  --sd_multiplyer $var --switch 2 --use_trees_as_input
