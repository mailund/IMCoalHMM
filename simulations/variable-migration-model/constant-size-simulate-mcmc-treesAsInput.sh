...
for sim in `eval echo {1..${no_sims}}`; do
	...
	out1="	python /home/svendvn/workspace/IMCoalHMM/scripts/variable-migration-model-mcmc.py -o INMmcmc-smallVar-sim-${sim}-chain.txt --treefile $treefile --samples 10 --thinning 1  --sd_multiplyer 0.05 --switch 2"
	python /home/svendvn/workspace/IMCoalHMM/scripts/variable-migration-model-mcmc.py -o INMmcmc-smallVar-sim-${sim}-chain.txt --treefile $treefile \
		--samples 10 --thinning 1  --sd_multiplyer 0.05 --switch 2 --use_trees_as_data
