...
for sim in `eval echo {1..${no_sims}}`; do
	...
	ms 4 1 -T -r ${coal_rho} ${seg_length} -I 2 2 2 -n 1 0.5 -g 1 1.0 -ma x ${sym_coal_mig_rate} $(bc -l <<< ${sym_coal_mig_rate}*0.5) x  | tail -n +4 | grep -v // > ${treefile}
	...
	echo $out2 >> INMmcmc-100d-mc3_15adap1-varPop-40states-${sim}-chain.txt
	echo "vi har ${no_chunks}*${seg_length} basepar" >>  INMmcmc-100d-mc3_15adap1-varPop-40states-${sim}-chain.txt

	cp /scratch/$SLURM_JOBID/INMmcmc-100d-mc3_15adap1-varPop-40states-${sim}-chain.txt /home/svendvn/
