...
for sim in `eval echo {1..${no_sims}}`; do
    ...
    for chunk in `eval echo {1..${no_chunks}}`; do
        ...
        python /scratch/$SLURM_JOBID/simulate_variable_migration_trees.py -o ${simdir}/ --type 1
	...
	out1="	python /scratch/$SLURM_JOBID/variable-migration-model-mcmc.py -o INMmcmc-$var-simTrees-${sim}-chain.txt --samples 10000 --thinning 10  --sd_multiplyer $var --switch 2"
	python /scratch/$SLURM_JOBID/variable-migration-model-mcmc.py -o INMmcmc-smallVar-sim-${sim}-chain.txt -a11 ${simdir}/*.11.ziphmm -a12 ${simdir}/*.12.ziphmm -a22 ${simdir}/*.22.ziphmm \
		--samples 2 --thinning 1  --sd_multiplyer 0.05
	echo $out1 >> INMmcmc-$var-simTrees-${sim}-chain.txt
	echo $out2 >> INMmcmc-$var-simTrees-${sim}-chain.txt
	cp /scratch/$SLURM_JOBID/INMmcmc-$var-simTrees-${sim}-chain.txt /home/svendvn/
