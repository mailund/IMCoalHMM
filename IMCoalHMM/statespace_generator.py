...
class IM(CoalSystem):
    ...
    def recombination(self, token):
        ...
        left, right = nucs
        if not (left and right): return []
        return [(pop,pop, iset([(pop,(left,iset())), (pop,(iset(),right))]))]
    ...
    def coalesce(self, token1, token2):
        ...
        left2, right2 = nuc2
        left, right = left1.union(left2), right1.union(right2)
        return pop1, pop2, iset([(pop1, (left, right))])
    ...
    def __init__(self, species):
        ...
        for s in species:
        	self.legal_migrations[s] = iset([ss for ss in species if s != ss])
        
        self.transitions = [[('R',self.recombination),('M',self.migrate)], 
        					[('C',self.coalesce)]]
...
if __name__ == "__main__":
    system = IM(range(2))
    states, transitions =  system.compute_state_space()
