...
FIXME: This requires a lot more work to both handle more general cases (variation in rates, handling periods with no migration, handle a final epoch in a single population, etc.) and optimisations to make it fast enough for proper use.
'''
...
def compute_transition_probabilities(ctmc, break_points):
    ...
    The time break points are the times between intervals, so there is one break point
    less than the number of states in the HMM.
    
    Returns both the stationary probability pi and the transition probability T, since 
    pi is autumatically calculated as part of the algorithm.
    
    '''
    
    # FIXME: Here I'm only handling a single epoch and only a migration system. It is a bit more
    # work, but not that hard to do, to generalize it.

    no_states = len(break_points) + 1
    initial = state_space.i12_index # FIXME: generalize this
    ...
    J[0,0] = P1[initial,ctmc.E_states].sum()
    ...
    for i in xrange(1,no_states-1):
        ...
        for b in ctmc.B_states:
            for e in ctmc.E_states:
                J[i,i] += P1[initial,b] * P2[b,e]
    
    P = ctmc.probability_matrix(break_points[-1])
    J[no_states-1,no_states-1] = P[initial,ctmc.B_states].sum()
    ...
    for j in xrange(1,no_states-1):
        ...
        prob = 0.0
        for l1 in ctmc.L_states:
            for l2 in ctmc.L_states:
                for e in ctmc.E_states:
