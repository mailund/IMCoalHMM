...
and uniform coalescence and recombination rates."""
    ...
    logL = Likelihood(VariableCoalescenceRateIsolationModel(), forwarder)
    intervals = [4] + [2]*25 + [4, 6]
    
    if options.est_split_time:
        minimizer = MinimizeWrapper(logL, intervals, True)
        ...
        if options.logfile:
            with open(options.logfile, 'w') as logfile:
                ...
                def transform(params):
                    ...
                    return [split] + [2/cr for cr in coal_rates] + [recomb_rate]
        ...
        minimizer = MinimizeWrapper(logL, intervals, False)
