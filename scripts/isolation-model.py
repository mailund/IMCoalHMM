...
init_coal = 1 / (theta / 2)
init_recomb = rho
init_parameters=[init_split, init_coal, init_recomb]
if options.outgroup:
    init_outgroup = init_split*3
    init_parameters.append(init_outgroup)
...
if options.emissionComplicated:
    if options.outgroup:
        base_log_likelihood = Likelihood(isolation_model2.IsolationModel(no_states, outgroup=True), forwarders)
    elif options.constant_break_points:
        base_log_likelihood = Likelihood(isolation_model2.IsolationModelConstantBreaks(no_states, breaktail=options.breakpoints_tail_pieces,
                                                                                         breaktimes=options.breakpoints_time, outgroup=False), forwarders)    
    else:
        base_log_likelihood = Likelihood(isolation_model2.IsolationModel(no_states, outgroup=False), forwarders)    

else:
    base_log_likelihood = Likelihood(IsolationModel(no_states), forwarders)
...
if options.optimizer=="Particle-Swarm":
    ...
    listOfTransforms=[]
    listOfTransforms.append(linear_transformfunc(0.03))#split.time
